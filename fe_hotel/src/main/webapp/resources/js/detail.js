 ...
 */
 ...
 var detailService= (function(){
	...
	function getList(param, callback, error) {
	    ...
	    $.getJSON("/hotel/pages/" + hotelno + "/" + page,
	        function(data) {
	...
	/* ===== 좋아하는 이유 CRUD ===== */
	function reasonList(param, callback, error) {
		var hotelno = param.hotelno;
		$.getJSON("/hotel/reason?hotelno=" + hotelno, function(data) {
	...
	function addReason(reason, callback, error) {
		$.ajax({
			type: 'post',
			url: '/hotel/reason',
	...
	function updateReason(reason, callback, error) {
		$.ajax({
			type: 'put',
			url: '/hotel/reason',
			data: JSON.stringify(reason),
	...
	function deleteReason(reasonNo, callback, error) {
		$.ajax({
			type: 'delete',
			url: '/hotel/reason/' + reasonNo,
			success: function(result) { if (callback) callback(result); },
			error: function(xhr, status, er) { if (error) error(er); }
		});
	}
 	...
 	return{
 		...
 		displayDate : displayDate, // util.js 전역 함수 참조
